...
var db = async () => {
  mongoose.connection.on("open", async function (ref) {
    console.log("Connected to mongo server. with create permission!");
    ...
    mongoose.connection.db
      ...
      .toArray(async function (err, collectionName) {
        for (var i = 0; i < collectionName.length; i++) {
          ...
          var findDeletePermission = await parmissionsModel.find(
            delete_permission
          );
          ...
          var findChangePermission = await parmissionsModel.find(
            change_permission
          );
