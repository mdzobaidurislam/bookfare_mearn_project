...
exports.registerCenter = async (req, res) => {
  try {
    ...
    if (!errors.isEmpty()) {
      ...
      // console.log(payload,"??????????????????????????");
      if (req.body.image) {
        payload["primaryImage"] = uploadBase64Image(
          req.body.image,
          "upload/images/",
          moment().format("DDMMYYhhiiss") + "image"
        );
      ...
      var message = ` (${
        center.name
      }) has been successfully registered with bookcare.in on (${moment().format(
        "DD-MM-YYYY"
      )}). You can now access our portal (<a href="${
        process.env.center_url
      }" target="blank">${
        process.env.center_url
      }</a>) using your login credentials:
        <br>
        ID: ${payload.email}
        Password: ${pswd}`;
...
exports.centerList = async (req, res) => {
  try {
    ...
    if (!verify_status) {
      return res
        .status(400)
        .json({ error: "verify_status requred req.query " });
    }
...
exports.centerDetails = async (req, res) => {
  try {
    ...
    const result = await centerDocumentsModel
      .find({ centerId: _id })
      .populate("document_typeId");
...
exports.centerUpdate = async (req, res) => {
  try {
    ...
    if (!errors.isEmpty()) {
      ...
      if (req.body.image) {
        options["primaryImage"] = uploadBase64Image(
          req.body.image,
          "upload/image/",
          "center_" + moment().format("DDMMYYhhiiss")
        );
      }
      // var coordinates = req.body.coordinates
      ...
      // options['location'] = {"coordinates":coordinates}
      console.log(options);
      await centerModel.findOneAndUpdate({ _id: _id }, options, {
        upsert: true,
      });
      ...
      await userModel.findOneAndUpdate({ center_id: _id }, options, {
        upsert: true,
      });
...
function updateMail(oldata, newdata, email) {
  ...
  if (
    newdata["insurance_billing_facility"] !==
    oldata["insurance_billing_facility"]
  ) {
...
exports.centerTimeUpdate = async (req, res) => {
  try {
    ...
    if (!errors.isEmpty()) {
      ...
      await centerModel
        .findOneAndUpdate({ "times._id": id }, { $set: { "times.$": payload } })
        .catch((err) => {
          res.status(500).json({
            error: err,
          });
...
exports.centerTimeAdd = async (req, res) => {
  try {
    ...
    if (!errors.isEmpty()) {
      ...
      var payload = req.body;
      const data = await centerModel.findOneAndUpdate(
        { _id: req.params.id },
        { $push: { times: payload } }
      );
      console.log(data);
  ...
  try {
    ...
    if (!errors.isEmpty()) {
      ...
      if (!user) {
        ...
        //     "location": "body"
        // }
        return res.status(423).json({ error: "User not found or match" });
      } else {
        const center = await centerModel.findOne({ _id: user.center_id });
        ...
        if (password === true) {
          ...
          await userModel.updateOne(
            { _id: user._id, UserType: "center" },
            { $push: { fcm_token: { token: payload.fcm_token } } }
          );
...
exports.centerProfile = async (req, res) => {
  try {
    ...
    const data = await centerModel
      .findOne({ _id: req.centerId })
      .catch((err) => {
        return res.status(500).json({ error: error });
      });
    return res.status(200).json({ data: data });
...
exports.CenterDocumentsUpload = async (req, res, next) => {
  try {
    ...
    if (req.body.document) {
      options["document"] = uploadBase64Image(
        req.body.document,
        "upload/images/",
        moment().format("DDMMYYhhiiss") + "image"
      );
...
exports.CenterDocumentsUploadDetails = async (req, res) => {
  try {
    ...
    const data = await centerDocumentsModel.find({
      _id: id,
      userId: req.centerId,
    });
    return res.status(200).json({ data: data });
...
exports.CenterPdfDocumentsUpload = async (req, res) => {
  try {
    ...
    if (req.body.document) {
      payload["document"] = uploadBase64Image(
        req.body.document,
        "upload/images/",
        moment().format("DDMMYYhhiiss") + "image"
      );
...
exports.centerDocumentCheck = async (req, res) => {
  try {
    const data = await centerDocumentsModel.findOne({
      document_typeId: req.params.id,
    });
...
exports.centerProfileEdit = async (req, res) => {
  try {
    ...
    if (!errors.isEmpty()) {
      ...
      var center = centerModel.findOne({
        _id: req.centerId ? req.centerId : req.params.id,
      });
...
exports.centerDocumentCategoryList = async (req, res) => {
  try {
    ...
    for (var i = 0; i < data.length; i++) {
      var check = await centerDocumentsModel.findOne({
        centerId: req.centerId,
        document_typeId: data[i]._id,
      });
...
exports.uploadGallery = async (req, res) => {
  try {
    ...
    for (var i = 0; i < images.length; i++) {
      var image = await uploadBase64Image(
        images[i],
        "upload/image/",
        "center_" + moment().format("DDMMYYhhiiss") + i + req.centerId
      );
