...
exports.patientsDetails = async (req, res) => {
  try {
    ...
    // if(modelType){
    var data = await familyMemberModel
      .findOne({ _id: _id })
      .populate("user_id");
    // console.log(data, "??????????");
    var appointments = await book_appointmentModel
      .find({
        patient_familyMemberId: data._id,
        center_id: req.centerId,
        appointment_status: { $ne: "not complete" },
      })
      .sort({ date: -1 });
    var reports = await patientsReportsModel.find({
      patientId: data._id,
      shareWith: { $elemMatch: { centerId: req.centerId } },
    });
    ...
    return res
      .status(200)
      .json({ data: data, appointments: appointments, reports: reports });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ error: error });
  }
  ...
  try {
    ...
    if (!errors.isEmpty()) {
      ...
      if (payload.profile_photo_path) {
        options["profile_photo_path"] = await uploadBase64Image(
          req.body.profile_photo_path,
          "upload/image/",
          "user_" + moment().format("DDMMYYhhiiss")
        );
      }
      ...
      if (payload.reports) {
        ...
        for (var i = 0; i < reports.length; i++) {
          if (reports[i].image) {
            ...
            options["file"] = await uploadBase64Image(
              reports[i].image,
              "upload/image/",
              "user_report" + moment().format("DDMMYYhhiiss") + i
            );
...
exports.patientUpdateFamilyMamber = async (req, res) => {
  try {
    ...
    if (!errors.isEmpty()) {
      ...
      if (payload.profile_photo_path) {
        payload["profile_photo_path"] = await uploadBase64Image(
          req.body.profile_photo_path,
          "upload/image/",
          "user_" + moment().format("DDMMYYhhiiss")
        );
...
exports.getAllCenterList = async (req, res) => {
  try {
    ...
    if (getQuery.price_from && getQuery.price_to) {
      querys["charges"] = {
        $gte: parseInt(getQuery.price_from),
        $lte: parseInt(getQuery.price_to),
      };
...
exports.appointmentSlotsDetails = async (req, res) => {
  ...
  try {
    ...
    const data = await date_slotsModel
      .find({
        center_id: centerId,
        date: { $gte: moment().format("DD-MM-YYYY") },
      })
      .select("date start_time end_time total_beds");
...
exports.dateSlotsList = async (req, res) => {
  try {
    ...
    if (date) {
      var currant_time = parseInt(
        moment().utcOffset("+05:30").format("HHmmss")
      );
      ...
      if (date == match_time) {
        ...
        var date_condition_match = {
          date: date,
          slot_start_time: { $gte: currant_time },
          center_id: mongoose.Types.ObjectId(req.params.id),
          left_bed: { $gte: 0 },
        };
      } else {
        var date_condition_match = {
    ...
    var data = await date_slotsModel
      .find({
        slot_date_n: { $gte: parseInt(moment().format("YYYYMMDD")) },
        center_id: mongoose.Types.ObjectId(req.params.id),
        left_bed: { $gte: -0 },
      })
      .where("left_bed")
...
exports.dateSlotsList1 = async (req, res) => {
  try {
    ...
    var data = await date_slotsModel
      .find({
        slot_date_n: { $gte: moment().format("DDMMYYYY") },
        center_id: mongoose.Types.ObjectId(req.params.id),
        left_bed: { $gte: -0 },
      })
      .where("left_bed")
...
exports.patientProfile = async (req, res) => {
  try {
    ...
    const appointments = await book_appointmentModel
      .find({ userId: profileId })
      .populate("center_id patient_familyMemberId")
      .sort({ date: -1 });
    const reports = await patientsReportsModel
      .find({ userId: profileId })
      .populate("patientId");
    ...
    const members = await familyMemberModel
      .find({ user_id: profileId })
      .sort({ createdAt: -1 });
...
exports.patientAppointment = async (req, res) => {
  try {
    ...
    const appointments = await book_appointmentModel
      .find({ userId: profileId, appointment_status: { $ne: "not complete" } })
      .populate("center_id patient_familyMemberId")
      .sort({ date: -1 });
...
exports.SendRegisterOTP = async (req, res) => {
  ...
  try {
    ...
    if (!errors.isEmpty()) {
      ...
      var check_otp = await verifyMobileModel.findOne({
        phone: payload.phone,
        userType: "patients",
      });
      console.log(check_otp);
      if (check_otp) {
        var check_otp = await verifyMobileModel.updateOne(
          { _id: check_otp._id },
          { otp: otp }
        );
      } else {
...
exports.newPatients = async (req, res) => {
  try {
    ...
    if (!errors.isEmpty()) {
      ...
      const payload = req.body;
      const check_otp = await verifyMobileModel.findOne({
        phone: payload.phone,
        otp: payload.otp,
        userType: "patients",
      });
      ...
      const newDataOTP = new verifyMobileModel(optionsOTP);
      await newDataOTP.save();
      await email_send(newData._id);

      var token = await jwtToken.sign(
        userDetails,
...
exports.bookAppointment = async (req, res) => {
  try {
    ...
    const slot = await date_slotsModel
      .findOne({ _id: payload.slot_id })
      .populate("center_id");

    var check_appointment = await book_appointmentModel.findOne({
      ...
      $or: [
        {
          apt_start_time: {
            $gte: slot.slot_start_time,
            $lt: slot.slot_end_time,
          },
        },
        {
          apt_endtime: { $gte: slot.slot_start_time, $lt: slot.slot_end_time },
        },
      ],
    });
    ...
    if (check_appointment) {
      return res.status(400).json({
        message:
          "You can't book appointment for this slot. Because you already have an appointment between this time",
      });
    }
    if (slot.left_bed <= 0) {
      return res
        .status(400)
        .json({ message: "Please select another slot, this slot is full" });
    }
    ...
    var commission =
      ((100 -
        parseInt(slot.center_id.commission ? slot.center_id.commission : "0")) /
        100) *
      parseInt(slot.center_id.charges);
    ...
    await date_slotsModel.updateOne(
      { _id: slot._id },
      { left_bed: leftBed, book_slots }
    );
    ...
    if (payload.payment_type != "online") {
      ...
      var message =
        "Your dialysis slot has been successfully booked at " +
        slot.center_id.name +
        " for " +
        slot.date +
        " at " +
      ...
      return res
        .status(200)
        .json({ mesaage: "create book", data: newData, notification });
    }
...
exports.bookAppointmentList = async (req, res) => {
  try {
    ...
    // console.log(querys);
    if (!page && !limit) {
      const allData = await book_appointmentModel
        .find(querys)
        .populate(populates)
        .sort({ apt_date: -1 });
...
exports.getAppointment = async (req, res) => {
  try {
    const data = await book_appointmentModel
      .findOne({ _id: req.params.appointmentId })
      .populate("center_id");
    return res.status(200).json({ data: data });
  } catch (error) {
    return res.status(500).json({ error });
...
exports.updateProfile = async (req, res) => {
  try {
    ...
    if (!errors.isEmpty()) {
      ...
      const patient_familyMemberId = await familyMemberModel.findOneAndUpdate(
        { user_id: userId, relation: "self" },
        payload
      );
      ...
      if (files) {
        ...
        for (var i = 0; i < files.length; i++) {
          if (files[i].image) {
            var option = {
              userId,
              file: await base64toImage(
                files[i].image,
                "upload/image/",
                "patients_reports_" + moment().format("DDMMYYhhiiss") + i
              ),
              title: files[i].title,
...
exports.bookAppointmentCancel = async () => {
  return new Promise(async (resolve, reject) => {
    const bookAppointment = await book_appointmentModel
      .find({ payment_status: "cancel" })
      .catch((err) => {
        reject(err);
      });
...
exports.cancelAppointment = async (req, res) => {
  try {
    ...
    var appointment = await book_appointmentModel.updateOne(
      { _id: id },
    ...
    var date_slot = await date_slotsModel.findOneAndUpdate(
      { _id: apt.slot_id },
      { left_bed: slot.left_bed + 1, book_slots: parseInt(slot.book_slots) - 1 }
    );
    let options = {
    ...
    var message =
      "As per your request, your Dialysis appointment for (" +
      slot.date +
      ", " +
      slot.start_time +
    ...
    return res
      .status(200)
      .json({ message: "cancelled appointment", data: notification });
  } catch (error) {
    appointment;
    console.log(error);
    return res.status(500).json({ error: error });
...
exports.rescheduleAppointment = async (req, res) => {
  try {
    ...
    if (check_appointment) {
      return res
        .status(423)
        .json({ message: "You already have an appointment between this time" });
    }
    ...
    const new_slot = await date_slotsModel
      .findOne({ _id: payload.slot_id })
      .populate("center_id");
    ...
    var commission =
      ((100 -
        parseInt(
          new_slot.center_id.commission ? new_slot.center_id.commission : "0"
        )) /
        100) *
      parseInt(new_slot.center_id.charges);
    ...
    date_slots = {
      ...
      apt_date: parseInt(
        moment(new_slot.date, "DD-MM-YYYY").format("YYYYMMDD")
      ),
    ...
    return res
      .status(200)
      .json({ message: "reschedule appointment", data: notification });
  } catch (error) {
    appointment;
    console.log(error);
    return res.status(500).json({ error: error });
...
exports.checkNotification = async (req, res) => {
  try {
    ...
    var notification_key = {
      ...
      registration_ids: [
        "eVSpIaFITUaoCaPR0kcG1p:APA91bGI4Yl-0v7Gi9cvn4w31V9zxe8ZbjnqJt0Se4I3PqDXt_-5CKZVQgXyOI9zytIYS1njtyUPS9ffB46MvS4yAXaS0ZGNuI3nyG5-ty6r4p0QYtE4v5KSwf8V1GIB2kUzAAVZ2BpK",
      ],
    };
    console.log(notification_key, "send");
    await axios
      .post("https://fcm.googleapis.com/fcm/send", notification_key, {
        headers: {
