...
exports.createOrder = async (req, res) => {
  try {
    ...
    const get_appointments = await book_appointmentModel
      .findOne({ _id, userId: user_id })
      .populate("userId center_id");
...
exports.createTransaction = async (req, res) => {
  try {
    ...
    if (payload.type == "appointment") {
      ...
      await newdata.save();
      let appointment = await book_appointmentModel.findOneAndUpdate(
      ...
      var apt = await book_appointmentModel.findOne({
      ...
      return res
        .status(200)
        .json({ mesaage: "create book", notification: notification });
