  ...
  try {
    ...
    if (!error.isEmpty()) {
      ...
      const user = await userModel.find({
        email: payload.email,
        // UserType: { $in: ["superadmin", "employee"] },
        // UserType: "superadmin",
      });
      console.log(user);
      ...
      if (!user) {
        ...
        const password = await passwordHash.verify(
          payload.password,
          user.password
        );
        if (password === true) {
...
exports.userRefreshToken = async (req, res) => {
  try {
    ...
    if (refreshToken in refreshTokens && refreshTokens[refreshToken] == email) {
      ...
      var token = await jwtToken.sign(user, process.env.JWT_SECREATE_kEY, {
        expiresIn: "86765m",
      });
      res.json({
        accessToken: token,
...
exports.forgotPasswordSendOTP = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(422)
        .json({ errors: [{ param: "phone", msg: "Invalid Credentials" }] });
    } else {
  ...
  try {
    ...
    if (!error.isEmpty()) {
      ...
      if (!user) {
        return res
          .status(422)
          .json({ errors: [{ param: "otp", msg: "Otp is invalid" }] });
      } else {
        let password = await passwordHash.generate(payload.password);

        var data = await userModel.findOneAndUpdate(
          { _id: user._id },
          { otp: null, password: password }
        );
...
exports.centerForgotPasswordSendOTP = async (req, res) => {
  try {
    let payload = req.body;
    console.log(payload);
    const center = await userModel
      .findOne({ phone: payload.phone, userType: "center" })
      .populate("center_id");
    if (!center) {
      return res
        .status(422)
      ...
      return res
        .status(200)
        .json({ message: "OTP successfully send", otp: otp });
    }
  } catch (error) {
    console.log(error);
    return res.status(500).json({ error });
