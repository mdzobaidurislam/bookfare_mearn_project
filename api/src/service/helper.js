    ...
    try {
      ...
      if (user.email) {
        ...
        var message = `Your registration is successfully 
                    Your OTP: ${user.otp}
      `;
        var resgisteration_no = user.resgisteration_no
          ? user.resgisteration_no
          : user.name;
        await mailSend.mail_send(
          resgisteration_no,
          user.email,
          subject,
          "any",
...
let pdfConvert = () => {
  fs.readdir("./upload/pdfconvert", (err, files) => {
    ...
    pdfDoc.image("./upload/pdfconvert/" + files[0], {
      fit: [300, 400],
      align: "center",
      valign: "center",
    });
...
let ImageValidate = (req, res, next) => {
  imageType = ["image/jpeg", "image/png", "image/jpg", "image/bmp"];
...
async function pushNotification(user_id, message) {
  try {
    ...
    const config = {
      ...
      headers: {
        Authorization:
          "key=AAAAq_Kr-y8:APA91bHBsUbgIPVpMGjM6h8jD1EZynQpuHuAXk8yTrqDP_lE2oknL9Rggt5pTUMa0_Bm8TCY8vPE81wBT_mJYTSGqkFXGCvrCzlBOddUYNeUZ4wFi5nUuSChsfnN03fwZZ_q88VuaClq",
      },
    };
