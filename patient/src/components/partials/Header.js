...
function Header() {
  ...
  function getNotifications() {
    axiosBaseUrl
      .get(`patients/api/notifications?page=1&limit=5`)
      .then((res) => {
  ...
  const handleOnIdle = (event) => {
    ...
    if (localStorage.getItem("activeUser")) {
      const interval = setInterval(() => {
        ...
        if (
          parseInt(moment().format("YYYYMMDDHHmmss")) -
            parseInt(moment(getLastActiveTime()).format("YYYYMMDDHHmmss")) >
          6000
        ) {
          if (localStorage.getItem("activeUser")) {
            logoutHandler();
  ...
  return (
    <>
      ...
      <header id="header" className="fixed-top align-items-center">
        <div className="container d-flex align-items-center justify-content-between">
          <div className="relative">
            <div className="logo">
              <Link to="/">
                <img src="img/logo.jpg" className="logo-img" width="100%" />
              </Link>
                ...
                <li>
                  <a
                    ...
                    onClick={() =>
                      window.scroll(
                        0,
                        document.getElementById("contact").offsetTop - 50
                      )
                    }
                  >
                    Contact Us
                  ...
                  <>
                    ...
                    <li>
                      <ul className="notification-drop">
                        <li className="item">
                          <i
                            className="fas fa-bell notification-bell"
                            aria-hidden="true"
                            onClick={() => {
                              setNotify(!notify);
                          ...
                          <ul
                            className={
                              notify && removeNotification
                                ? "card-body card notification-list h-100p"
                                : "card-body card notification-list h-0"
                            }
                          >
                              ...
                              .map((notification, index) => {
                                return (
                                  <>
                                    <li className="enwwu">
                                      <Link
                                        key={index + "head-notify"}
                                        to={`/Profile/${notification.module}`}
                                      >
                                        <span>
                                        ...
                                        <br />
                                        <span className="notifictaion-time">
                                          {notification.date}
                                        </span>
                                      </Link>
                                    </li>
                            ...
                            <li className="text-center">
                              ...
                              <Link
                                ...
                                className="load-more-btn mt-2 mb-3"
                              >
                                Load More
                              </Link>{" "}
                            </li>
                    ...
                    <li>
                      <Link to="/Profile">
                        <p className="profile-icon-header">
                          {userName
                            ?.match(/(\b\S)?/g)
                            .join("")
                            .match(/(^\S|\S$)?/g)
          ...
          <ul className={mobileMenu ? "mobile-menu w-50" : "mobile-menu "}>
            ...
            {localStorage.getItem("activeUser") != undefined ? (
              <>
                ...
                <li className="nav-item">
                  <a onClick={logoutHandler} className="nav-link" to="/login">
                    Logout
                  </a>
                </li>
              </>
        ...
        {location.pathname.includes("centre/") ||
        location.pathname.includes("/editProfile") ||
        location.pathname.includes("editFamily") ||
        location.pathname.includes("AddFamily") ? (
          <div
            className="back-header "
            onClick={() => setremoveNotification(false)}
          >
      ...
      {loginState ? (
        <LoginModal showModal={loginState} setShowModal={setLoginState} />
      ) : (
        ""
      )}
